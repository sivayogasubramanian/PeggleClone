...
class LinePhysicsBodyTests: XCTestCase {
    func testConstructor() {
        let line = LinePhysicsBody(start: CGVector(dx: 1, dy: 2), end: CGVector(dx: 3, dy: 4))
        XCTAssertNotNil(line)
    }

    func testClosestPointOnLine_whenLineVertical() {
        let verticalLine = LinePhysicsBody(start: CGVector(dx: 3, dy: 2), end: CGVector(dx: 3, dy: 4))
    ...
    func testClosestPointOnLine_whenLineHorizontal() {
        let horizontalLine = LinePhysicsBody(start: CGVector(dx: 1, dy: 2), end: CGVector(dx: 3, dy: 2))
        let point = CGVector(dx: 5, dy: 4)
    ...
    func testClosestPointOnLine_withPositivePoints() {
        let line = LinePhysicsBody(start: CGVector(dx: 1, dy: 2), end: CGVector(dx: 3, dy: 7))
        let point = CGVector(dx: 5, dy: 4)
    ...
    func testClosestPointOnLine_withNegativePoints_forLine() {
        let line = LinePhysicsBody(start: CGVector(dx: -2, dy: -2), end: CGVector(dx: 3, dy: 3))
